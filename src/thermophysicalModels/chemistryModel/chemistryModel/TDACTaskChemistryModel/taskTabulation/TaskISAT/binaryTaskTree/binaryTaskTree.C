...
void Foam::binaryTaskTree<CompType, ThermoType>::insertNode
(
    chemPoint*& phi0,
    node*& newNode
)
...
template<class CompType, class ThermoType>
bool Foam::binaryTaskTree<CompType, ThermoType>::inSubTree
(
...
template<class CompType, class ThermoType>
void Foam::binaryTaskTree<CompType, ThermoType>::deleteSubTree(node* subTreeRoot)
{
...
template<class CompType, class ThermoType>
void Foam::binaryTaskTree<CompType, ThermoType>::transplant(node* u, node* v)
{
    if (v != nullptr)
    {
        // u is root_
...
template<class CompType, class ThermoType>
Foam::chemPointTaskISAT<CompType, ThermoType>*
Foam::binaryTaskTree<CompType, ThermoType>::chemPSibling(node* y)
{
    if (y->parent() != nullptr)
    {
    ...
    // the binaryTaskNode is root_ and has no sibling
    return nullptr;
}
...
template<class CompType, class ThermoType>
Foam::chemPointTaskISAT<CompType, ThermoType>*
Foam::binaryTaskTree<CompType, ThermoType>::chemPSibling(chemPoint* x)
{
    if (size_ > 1)
    {
...
template<class CompType, class ThermoType>
Foam::binaryTaskNode<CompType, ThermoType>*
Foam::binaryTaskTree<CompType, ThermoType>::nodeSibling(node* y)
{
    if (y->parent() != nullptr)
    {
...
template<class CompType, class ThermoType>
Foam::binaryTaskNode<CompType, ThermoType>*
Foam::binaryTaskTree<CompType, ThermoType>::nodeSibling(chemPoint* x)
{
    if (size_ > 1)
    {
        if (x == x->node()->leafLeft())
...
template<class CompType, class ThermoType>
void Foam::binaryTaskTree<CompType, ThermoType>::deleteAllNode(node* subTreeRoot)
{
    if (subTreeRoot != nullptr)
    {
...
template<class CompType, class ThermoType>
Foam::binaryTaskTree<CompType, ThermoType>::binaryTaskTree
(
...
template<class CompType, class ThermoType>
Foam::label Foam::binaryTaskTree<CompType, ThermoType>::depth(node* subTreeRoot)
{
...
template<class CompType, class ThermoType>
void Foam::binaryTaskTree<CompType, ThermoType>::insertNewLeaf
(
    const scalarField& phiq,
...
)
{
    ...
    else // at least one point stored
    {
        ...
        if (phi0 == nullptr)
        {
            binaryTaskTreeSearch(phiq, root_, phi0);
        }
        ...
        // the hyper plane is computed in the binaryTaskNode constructor
        node* newNode;
        if (size_ > 1)
        {
        ...
        {
            // when size is 1, the binaryTaskNode is without hyperplane
...
template<class CompType, class ThermoType>
void Foam::binaryTaskTree<CompType, ThermoType>::binaryTaskTreeSearch
(
    const scalarField& phiq,
    node* node,
    chemPoint*& nearest
)
{
    if (size_ > 1)
    {
        ...
        if (vPhi > a) // on right side (side of the newly added point)
        {
            ...
            {
                binaryTaskTreeSearch(phiq, node->nodeRight(), nearest);
            }
        ...
        else // on left side (side of the previously stored point)
        {
            ...
            {
                binaryTaskTreeSearch(phiq, node->nodeLeft(), nearest);
            }
...
template<class CompType, class ThermoType>
bool Foam::binaryTaskTree<CompType, ThermoType>::secondaryBTSearch
(
    const scalarField& phiq,
    chemPoint*& x
)
...
template<class CompType, class ThermoType>
void Foam::binaryTaskTree<CompType, ThermoType>::deleteLeaf(chemPoint*& phi0)
{
...
template<class CompType, class ThermoType>
void Foam::binaryTaskTree<CompType, ThermoType>::balance()
{
...
template<class CompType, class ThermoType>
Foam::chemPointTaskISAT<CompType, ThermoType>*
Foam::binaryTaskTree<CompType, ThermoType>::treeMin(node* subTreeRoot)
{
    if (subTreeRoot != nullptr)
    {
...
template<class CompType, class ThermoType>
Foam::chemPointTaskISAT<CompType, ThermoType>*
Foam::binaryTaskTree<CompType, ThermoType>::treeSuccessor(chemPoint* x)
{
    if (size_ > 1)
    {
        if (x == x->node()->leafLeft())
...
template<class CompType, class ThermoType>
void Foam::binaryTaskTree<CompType, ThermoType>::clear()
{
...
template<class CompType, class ThermoType>
bool Foam::binaryTaskTree<CompType, ThermoType>::isFull()
{
...
template<class CompType, class ThermoType>
void Foam::binaryTaskTree<CompType, ThermoType>::resetNumRetrieve()
{
