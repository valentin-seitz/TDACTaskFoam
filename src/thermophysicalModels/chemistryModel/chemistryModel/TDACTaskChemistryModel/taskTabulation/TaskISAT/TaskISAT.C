...
Foam::chemistryTaskTabulationMethods::TaskISAT<CompType, ThermoType>::TaskISAT
(
    const dictionary& chemistryProperties,
    TDACTaskChemistryModel<CompType, ThermoType>& chemistry
)
:
    chemistryTaskTabulationMethod<CompType, ThermoType>
    (
        chemistryProperties,
        chemistry
    ),
    ...
    cleaningRequired_(false)
{
    ...
    if (this->log())
    {
        nRetrievedFile_ = chemistry.logFile("found_TaskISAT.out");
        nGrowthFile_ = chemistry.logFile("growth_TaskISAT.out");
        nAddFile_ = chemistry.logFile("add_TaskISAT.out");
        sizeFile_ = chemistry.logFile("size_TaskISAT.out");
    }
}
...
template<class CompType, class ThermoType>
void Foam::chemistryTaskTabulationMethods::TaskISAT<CompType, ThermoType>::addToMRU
(
    chemPointTaskISAT<CompType, ThermoType>* phi0
)
{
...
template<class CompType, class ThermoType>
void Foam::chemistryTaskTabulationMethods::TaskISAT<CompType, ThermoType>::calcNewC
(
    chemPointTaskISAT<CompType, ThermoType>* phi0,
...
template<class CompType, class ThermoType>
bool Foam::chemistryTaskTabulationMethods::TaskISAT<CompType, ThermoType>::grow
(
    chemPointTaskISAT<CompType, ThermoType>* phi0,
    const scalarField& phiq,
...
)
{
    ...
    // The actual solution and the approximation given by TaskISAT are too different
    return false;
}
...
template<class CompType, class ThermoType>
bool
Foam::chemistryTaskTabulationMethods::TaskISAT<CompType, ThermoType>::cleanAndBalance()
{
    ...
    chemPointTaskISAT<CompType, ThermoType>* x = chemisTree_.treeMin();
    while (x != nullptr)
    {
        chemPointTaskISAT<CompType, ThermoType>* xtmp =
...
template<class CompType, class ThermoType>
void Foam::chemistryTaskTabulationMethods::TaskISAT<CompType, ThermoType>::computeA
(
...
template<class CompType, class ThermoType>
bool Foam::chemistryTaskTabulationMethods::TaskISAT<CompType, ThermoType>::retrieve
(
...
)
{
    ...
    if (chemisTree_.size())
    {
        chemisTree_.binaryTaskTreeSearch(phiq, chemisTree_.root(), phi0);
...
template<class CompType, class ThermoType>
Foam::label Foam::chemistryTaskTabulationMethods::TaskISAT<CompType, ThermoType>::add
(
    const scalarField& phiq,
    const scalarField& Rphiq,
    const scalar rho,
...
)
{
    ...
    if (chemisTree().isFull())
    {
        ...
        if (!cleanAndBalance())
        {
            DynamicList<chemPointTaskISAT<CompType, ThermoType>*> tempList;
            if (maxMRUSize_>0)
            {
                // Create a copy of each chemPointTaskISAT of the MRUList_ before
                // they are deleted
                forAllConstIters(MRUList_, iter)
                {
                    tempList.append
                    (
                        new chemPointTaskISAT<CompType, ThermoType>(*iter())
                    );
                }
            }
            chemisTree().clear();
            ...
            chemPointTaskISAT<CompType, ThermoType>* nulPhi = nullptr;
            for (auto& t : tempList)
            {
    ...
    (
        ...
        this->tolerance(),
        scaleFactor_.size(),
        lastSearch_ // lastSearch_ may be nullptr (handled by binaryTaskTree)
    );
...
template<class CompType, class ThermoType>
void
Foam::chemistryTaskTabulationMethods::TaskISAT<CompType, ThermoType>::writePerformance()
{
    if (this->log())
    {
        nRetrievedFile_()
